package hw.ch10;

// 상수를 정의하는 클래스
public enum Hand {
    // 가위 바위 보를 나타내는 세 개의 enum 상수
    // 각 상수는 Hand 객체를 생성해서 가짐 => 생성자를 호출
    ROCK("바위", 0),
    SCISSORS("가위", 1),
    PAPER("보", 2);

    // enum이 가진 필드 
    private String name; // 가위 바위 보 손의 이름
    private int handvalue; // 가위 바위 보 손의 값 

    // 손의 값으로 상수를 얻기 위한 배열
    private static Hand[] hands = {
        ROCK, SCISSORS, PAPER
    };  // static: Hand 클래스에 숫자를 넘겨 상수를 얻는 메소드에서 hands 배열을 사용할 수 있도록

    // 생성자 
    private Hand(String name, int handvalue) {
        this.name = name;
        this.handvalue = handvalue;
    }

    // 손의 값으로 enum 상수를 가져온다 
    public static Hand getHand(int handvalue) {
        return hands[handvalue];
    }

    // this가 h보다 강할 때 true
    public boolean isStrongerThan(Hand h) {
        return fight(h) == 1;
    }
    // 현재 손은 this, 상대방의 손은 h, fight 메소드는 this와 h를 비교해서 무승부는 0, this가 이기면 1, h가 이기면 -1을 반환

    // this가 h보다 약할 때 true
    public boolean isWeakerThan(Hand h) {
        return fight(h) == -1;
    }

    // 무승부는 0, this가 이기면 1, h가 이기면 -1
    private int fight(Hand h) {
        if (this == h) {    // 객체 주소가 동일한 비교 => 같은 객체인지 비교
            return 0;
        } else if ((this.handvalue + 1) % 3 == h.handvalue) {
            return 1;
        } else {
            return -1;
        }
    }

    // 가위 바위 보의 문자열 표현
    @Override
    public String toString() {
        return name;
    }
}
